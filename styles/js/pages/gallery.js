      ...
      for (let entry of arr) {
        ...
        for (let id of entry.picturedcharacters) {
          entry.nameBadges.push(
            `<a class="badge badge-primary" href="${charadex.url.addUrlParameters(charadex.url.getPageUrl('characters'), {profile: id.trim()})}">${( (typeof charadex !== 'undefined' && charadex.database?.characters) || window.characters || [] ).find(c => c.id == id.trim())?.name || id.trim()}</a>`
          );
        }
